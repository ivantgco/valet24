(function () {
	...
	var one_action_map;
	var sid = MB.User.sid;
	var hs_res = contentInstance.params['hall_scheme_res'];
	var fz_id = hs_res[0]['FUND_ZONE_ID'];
	var fz_title = hs_res[0]['FUND_ZONE'];
	var showUnused = true;
	...
	var o = {
		...
		sid: sid,
		params: {
			fund_zone_id: contentInstance.fund_zone_id
		}
	};
	var layerO = {
		command: "get",
...
//        one_action_map.resize();
	...
	contentWrapper.find('.change-fund-zone-scheme').off('click').on('click', function () {
		MB.Core.switchModal({
			type: 'form',
			isNewModal: true,
			...
			params: {
				setFundZoneSchemeCb: function (id, title) {
//                    console.warn('CB', id, title);
					environment.fund_zone_id = id;
					environment.fund_zone_title = title;
					one_action_map.loadObj.params.fund_zone_id = id;
					one_action_map.reLoad();
					fundZones.load();
				}
			}
