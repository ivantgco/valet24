...
DefaultSettings.prototype = {
    ...
    colorpickerClassName: 'colorpickerInTable',
...
(function (Handsontable) {
	...
	Handsontable.renderers.cellDecorator = function (instance, TD, row, col, prop, value, cellProperties) {
        ...
        if(cellProperties.isColorpicker === "TRUE"){
            instance.view.wt.wtDom.addClass(TD, cellProperties.colorpickerClassName);
        }
...
(function (Handsontable) {
    ...
    var ColorpickerRenderer = function (instance, TD, row, col, prop, value, cellProperties) {
        ...
        var escaped = '<div class="colorView" style="background-color: '+value+'"></div>'+ Handsontable.helper.stringify(value);
        ...
        if (cellProperties.rendererTemplate) {
            ...
            instance.view.wt.wtDom.fastInnerHTML(TD, escaped); //this is faster than innerHTML. See: https://github.com/warpech/jquery-handsontable/wiki/JavaScript-&-DOM-performance-tips
        }

    };

    //Handsontable.TextRenderer = TextRenderer; //Left for backward compatibility
    Handsontable.renderers.ColorpickerRenderer = ColorpickerRenderer;
    Handsontable.renderers.registerRenderer('colorpicker', ColorpickerRenderer);
...
(function(Handsontable){
    var ColorpickerEditor = Handsontable.editors.BaseEditor.prototype.extend();

    ColorpickerEditor.prototype.init = function(){
        this.createElements();
        this.bindEvents();
    };

    ColorpickerEditor.prototype.getValue = function(){
        return this.INPUT.value
    };

    ColorpickerEditor.prototype.setValue = function(newValue){
    ...
    var onBeforeKeyDown =  function onBeforeKeyDown(event){
        ...
        //Process only events that have been fired in the editor
        if (event.target !== that.INPUT || event.isImmediatePropagationStopped()){
            return;
        }
        ...
        switch (event.keyCode) {
            case keyCodes.ARROW_RIGHT:
                if (that.wtDom.getCaretPosition(that.INPUT) !== that.INPUT.value.length) {
                    event.stopImmediatePropagation();
                }
                break;

            case keyCodes.ARROW_LEFT: /* arrow left */
                if (that.wtDom.getCaretPosition(that.INPUT) !== 0) {
                    event.stopImmediatePropagation();
                }
                break;
    ...
    ColorpickerEditor.prototype.open = function(){
        this.refreshDimensions(); //need it instantly, to prevent https://github.com/warpech/jquery-handsontable/issues/348

        this.instance.addHook('beforeKeyDown', onBeforeKeyDown);
    };

    ColorpickerEditor.prototype.close = function(){
        this.inputParentStyle.display = 'none';
    ...
    ColorpickerEditor.prototype.focus = function(){
        this.INPUT.focus();
        this.wtDom.setCaretPosition(this.INPUT, this.INPUT.value.length);
    };

    ColorpickerEditor.prototype.createElements = function(){
        this.$body = $(document.body);
        this.wtDom = new WalkontableDom();
        ...
        this.wtDom.addClass(this.INPUT, 'handsontableInput');

        this.inputStyle = this.INPUT.style;
        this.inputStyle.width = 0;
        this.inputStyle.height = 0;

        this.INPUT_PARENT = document.createElement('DIV');
        this.wtDom.addClass(this.INPUT_PARENT, 'handsontableInputHolder');

        this.inputParentStyle = this.INPUT_PARENT.style;
        this.inputParentStyle.top = 0;
        this.inputParentStyle.left = 0;
        this.inputParentStyle.display = 'none';

        this.INPUT_PARENT.appendChild(this.INPUT);

        this.instance.rootElement[0].appendChild(this.INPUT_PARENT);
    ...
    ColorpickerEditor.prototype.refreshDimensions = function () {
        if (this.state !== Handsontable.EditorState.EDITING) {
            return;
        }
        ...
        this.inputParentStyle.top = editTop + 'px';
        this.inputParentStyle.left = editLeft + 'px';
        ///end prepare textarea position
        ...
        this.inputParentStyle.display = 'block';
    };

    ColorpickerEditor.prototype.bindEvents = function(){
        var that = this;
        this.$input.on('cut.editor', function (event) {
            event.stopPropagation();
        });
        this.$input.on('paste.editor', function (event) {
            event.stopPropagation();
        });
        this.$input.colorpicker();

        this.$input.on('changeColor', function(ev){
            var color = ev.color.toHex();
            that.setValue(color);
        });
    };

    Handsontable.editors.ColorpickerEditor = ColorpickerEditor;
    Handsontable.editors.registerEditor('colorpicker', Handsontable.editors.ColorpickerEditor);
...
// Sargon's datetimeeditor extention
(function (Handsontable) {
	...
	DatetimeEditor.prototype.createElements = function () {
		...
		this.instance.view.wt.wtDom.addClass(this.datetimePicker, 'htDatetimepickerHolder');
		...
		this.$datetimePicker.on('changeDate', function (e) {
			var timestamp = e.date.valueOf() - 14400000 // - 4 hours
	...
	DatetimeEditor.prototype.beginEditing = function (row, col, prop, useOriginalValue, suffix) {
		Handsontable.editors.TextEditor.prototype.beginEditing.apply(this, arguments);
	...
	DatetimeEditor.prototype.showDatetimepicker = function () {
		var $td = $(this.TD);
		var offset = $td.offset();
		this.datetimePickerStyle.top = (offset.top + $td.height()) + 'px';
		this.datetimePickerStyle.left = offset.left + 'px';
		var datetimeOptions = {
			defaultDatetime: this.originalValue || void 0
		};
		$.extend(datetimeOptions, this.cellProperties);
		// this.$datetimePicker.datetimepicker("option", datetimeOptions);
		if (this.originalValue) {
			this.$datetimePicker.datetimepicker('setStartDate', this.originalValue);
		}
		this.datetimePickerStyle.display = 'block';
		// this.$datetimePicker.datetimepicker('show');
	};
	DatetimeEditor.prototype.hideDatetimepicker = function () {
...
// (function (Handsontable) {
// 	var ColorpickerEditor = Handsontable.editors.TextEditor.prototype.extend();
// 	ColorpickerEditor.prototype.init = function () {
// 		if (!$.fn.colorpicker) {
// 			throw new Error("jQuery UI Colorpicker dependency not found. Did you forget to include jquery-ui.custom.js or its substitute?");
// 		}
// 		Handsontable.editors.TextEditor.prototype.init.apply(this, arguments);
// 		this.isCellEdited = false;
// 		var that = this;
...
// 		this.instance.view.wt.wtDom.addClass(this.colorpicker, 'htColorpickerHolder');
// 		this.colorpickerStyle = this.colorpicker.style;
// 		this.colorpickerStyle.position = 'absolute';
// 		this.colorpickerStyle.top = 0;
// 		this.colorpickerStyle.left = 0;
// 		this.colorpickerStyle.zIndex = 99;
// 		var input = document.createElement('INPUT');
// 		this.colorpicker.appendChild(input);
// 		this.$colorpicker = $(this.colorpicker).find("input");
// 		this.$colorpicker.colorpicker();
// 	};
// 	ColorpickerEditor.prototype.showColorpicker = function () {
// 		var $td = $(this.TD);
// 		var offset = $td.offset();
// 		this.colorpickerStyle.top = (offset.top + $td.height()) + 'px';
// 		this.colorpickerStyle.left = offset.left + 'px';
// 		var colorpickerOptions = {
// 			defaultColorpicker: this.originalValue || void 0
// 		};
// 		$.extend(colorpickerOptions, this.cellProperties);
// 		this.$colorpicker.colorpicker("setValue", colorpickerOptions);
// 		if (this.originalValue) {
// 			this.$colorpicker.colorpicker('setValue', this.originalValue);
// 		}
// 		this.colorpickerStyle.display = 'block';
// 	};
// 	Handsontable.editors.ColorpickerEditor = ColorpickerEditor;
// 	Handsontable.editors.registerEditor('colorpicker', ColorpickerEditor);
// })(Handsontable);
...
// 	};
	// var ColorpickerEditor = Handsontable.editors.TextEditor.prototype.extend();
	// ColorpickerEditor.prototype.init = function () {
	// 	console.log("ColorpickerEditor.prototype.init");
	// 	if (!$.fn.colorpicker) {
	// 		throw new Error("jQuery UI Colorpicker dependency not found. Did you forget to include jquery-ui.custom.js or its substitute?");
	// 	}
	// 	Handsontable.editors.TextEditor.prototype.init.apply(this, arguments);
	...
	// 	Handsontable.editors.TextEditor.prototype.createElements.apply(this, arguments);
	// 	var $td = $(this.TD);
	// 	$td.html('<div class="input-append color" data-color="rgb(255, 146, 180)" data-color-format="rgb"><input type="text" class="span2" value="" ><span class="add-on"><i style="background-color: rgb(255, 146, 180)"></i></span></div>');
	// 	this.$colorpicker = $td.find(".color");
	// 	this.$colorpicker.colorpicker();
	// 	// this.colorpicker = document.createElement('DIV');
	// 	// this.instance.view.wt.wtDom.addClass(this.colorpicker, 'htColorpickerHolder');
	// 	// this.colorpickerStyle = this.colorpicker.style;
	...
	// 	// 	format: "hex"
	// 	// };
	// 	// this.$colorpicker.colorpicker(defaultOptions);
	...
	// 	// });
	// 	// // Prevent recognizing clicking on jQuery Datepicker as clicking outside of table
	// 	// this.$colorpicker.on('mousedown', function (event) {
	// 	// 	event.stopPropagation();
	// 	// });
	// 	// this.hideColorpicker();
	// };
	// ColorpickerEditor.prototype.destroyElements = function () {
	// 	this.$colorpicker.colorpicker('hide');
	// 	this.$colorpicker.remove();
	// };
	// ColorpickerEditor.prototype.beginEditing = function (row, col, prop, useOriginalValue, suffix) {
	// 	Handsontable.editors.TextEditor.prototype.beginEditing.apply(this, arguments);
	// 	this.showColorpicker();
	// };
	// ColorpickerEditor.prototype.finishEditing = function (isCancelled, ctrlDown) {
	// 	this.hideColorpicker();
	// 	Handsontable.editors.TextEditor.prototype.finishEditing.apply(this, arguments);
	// };
	// ColorpickerEditor.prototype.showColorpicker = function () {
	...
	// 	// // 	defaultColorpicker: this.originalValue || void 0
	// 	// // };
	// 	// // console.log("colorpickerOptions", colorpickerOptions);
	// 	// // $.extend(colorpickerOptions, this.cellProperties);
	// 	// // console.log("colorpickerOptions", colorpickerOptions);
	// 	// // this.$colorPicker.colorpicker("setValue", datetimeOptions);
	// 	// // if (this.originalValue) {
	// 	// // 	this.$datetimePicker.datetimepicker("setDatetime", this.originalValue);
	// 	// // 	this.$colorpicker.colorpicker('setValue', this.originalValue);
	// 	// // }
	// 	// // this.colorpicker
	// 	// this.colorpickerStyle.display = 'block';
	// 	this.$colorpicker.show();
	// 	this.$colorpicker.colorpicker('show');
	// };
	// ColorpickerEditor.prototype.hideColorpicker = function () {
	// 	this.colorpickerStyle.display = 'none';
	// 	this.$colorpicker.colorpicker('hide');
	// };
	// Handsontable.editors.ColorpickerEditor = ColorpickerEditor;
	// Handsontable.editors.registerEditor('colorpicker', ColorpickerEditor);
...
(function (Handsontable) {
	...
	MultiselectEditor.prototype.init = function () {
		...
		if (!$.fn.select2) {
			throw new Error("jQuery UI select2 dependency not found. Did you forget to include jquery-ui.custom.js or its substitute?");
		}
	...
	MultiselectEditor.prototype.createElements = function () {
		...
		Handsontable.editors.TextEditor.prototype.createElements.apply(this, arguments);
		this.multiselect = document.createElement('INPUT');
		this.instance.view.wt.wtDom.addClass(this.multiselect, 'htMultiselectHolder');
		this.multiselectStyle = this.multiselect.style;
		this.multiselectStyle.position = 'absolute';
		this.multiselectStyle.top = 0;
		this.multiselectStyle.left = 0;
		this.multiselectStyle.zIndex = 99;
		document.body.appendChild(this.multiselect);
		...
		this.$multiselect.select2(defaultOptions);
		// this.$datetimePicker.on('changeDate', function (e) {
		// 	var timestamp = e.date.valueOf() - 14400000 // - 4 hours
		// 	var date = new Date(timestamp);
		// 	var dd = date.getDate(),
	...
	MultiselectEditor.prototype.destroyElements = function () {
		this.$multiselect.select2('destroy');
		this.$multiselect.remove();
	};

	MultiselectEditor.prototype.beginEditing = function (row, col, prop, useOriginalValue, suffix) {
		Handsontable.editors.TextEditor.prototype.beginEditing.apply(this, arguments);
		this.showMultiselect();
	};

	MultiselectEditor.prototype.finishEditing = function (isCancelled, ctrlDown) {
		this.hideMultiselect();
		Handsontable.editors.TextEditor.prototype.finishEditing.apply(this, arguments);
	};

	MultiselectEditor.prototype.showMultiselect = function () {
		var $td = $(this.TD);
		var offset = $td.offset();
		this.multiselectStyle.top = (offset.top + $td.height()) + 'px';
		this.multiselectStyle.left = offset.left + 'px';
		var datetimeOptions = {
			defaultMultiselect: this.originalValue || void 0
		};
		$.extend(datetimeOptions, this.cellProperties);
		// this.$datetimePicker.datetimepicker("option", datetimeOptions);
		if (this.originalValue) {
			this.$multiselect.select2('val', this.originalValue);
		}
		this.multiselectStyle.display = 'block';
		// this.$datetimePicker.datetimepicker('show');
	};

	MultiselectEditor.prototype.hideMultiselect = function () {
		this.multiselectStyle.display = 'none';
		// this.$datetimePicker.datetimepicker('hide');
	};
...
Handsontable.ColorpickerCell = {
    editor: Handsontable.editors.ColorpickerEditor,
    renderer: Handsontable.renderers.ColorpickerRenderer
};
...
Handsontable.MultiselectCell = {
	editor: Handsontable.editors.MultiselectEditor,
	renderer: Handsontable.renderers.AutocompleteRenderer
};
...
Handsontable.cellTypes = {
	...
	colorpicker: Handsontable.ColorpickerCell,
	...
	// Sargon's update
	datetime: Handsontable.DatetimeCell,
	multiselect: Handsontable.MultiselectCell
	// colorpicker: Handsontable.ColorpickerCell
};
