...
var Tribunes = function(){
	this.list = [];
};
var Tribune = function(params){
	this.title = params.title || "Зона";
	this.tribune_id = params.tribune_id || undefined;
	this.selected = false;
	this.expanded = false;
	this.zones = [];
};
...
var Zone = function(params){
    ...
    this.tribune_id = params.tribune_id || undefined;
    this.placesCount = params.placesCount || 0;
    this.selected = false;
    this.ids = [];
};
...
Tribunes.prototype = {
	findTribune: function(id){
		for(var i=0; i<this.list.length; i++ ){
			if( this.list[i].tribune_id == id){
				return this.list[i]
			}
		}
	},
	addTribune: function(item){
		this.list.push(item);
	},
	removeTribune: function(id){
		for(var i=0; i< this.list.length; i++){
			var item = this.list[i];
			if(item.tribune_id == id){
				delete this.list[i];
			}
		}
		this.clearEmpty();
...
Zones.prototype = {
	...
	findZonesByTribune: function(id){
		var zones = [];

		for(var i=0; i<this.list.length; i++ ){
			if( this.list[i].tribune_id == id){
				zones.push(this.list[i]);
			}
		}
