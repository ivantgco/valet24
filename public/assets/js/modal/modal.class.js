...
MBOOKER.ModalLib ={
    ruName: {
        modal_test:"Test win modal",
        modal_test2:"Test win modal 2"
    },
    iIcon: {
        modal_test: "fa-rub",
        modal_test2: ""
...
var ModalWin_tpl = function () {
	var _this = this;
    var MadalPath = '';
	var ModalDiv = document.getElementById( 'modal_win' );
	var ModalObj = $( '#ModalWin' );
	...
	this.init = function(){
		...
		ModalObj.find("#modal_win").append(html);
    ...
    this.CreateContent = function (ids,modal_obj) {
    	var tabs = {};
    	var exContent = 0;
    	...
    	if(exContent == 0){
            var modal_name = MBOOKER.ModalLib.ruName[modal_obj];
            var modal_icon = MBOOKER.ModalLib.iIcon[modal_obj];
	    	// Генерация HTML
	    	var htmlTopMenu = ''+
	    	...
	    	thisContent.load(MadalPath+''+modal_obj+".html",function(){
	    		$(this).prepend('<i class="close_content fa fa-times-circle"></i>');
	    		$('#modal_content_'+modal_obj+' i.close_content').click(function(){
...
var ModalWin = new ModalWin_tpl();


function modal_show(ids,modal_obj){
	ModalWin.trigger();
	ModalWin.CreateContent(ids,modal_obj);
}
function modal_createContent(ids,modal_obj){
	ModalWin.CreateContent(ids,modal_obj);
...
$(document).ready(function(){
	$("a.open").click(function(){
		modal_show({"0":"1","1":"25"},"modal_test");
	})
})
